...
from .core.bokeh import RollingResult, date_figure, rolling
from .core.pandas import resample_sum, unlocalize
...
def _plot_manual_exercise(df):
    ...
    groups = sorted(groups, key=lambda kind_d: max(unlocalize(kind_d[1]['dt'])), reverse=True)

    kinds = [kind for kind, _ in groups]
    # make colors stable
    colors = dict(zip(kinds, cycle(pallete)))
    colors['errors'] = 'red'
    ...
    for k, edf in chain(
        ...
        m_x_range: dict[str, Any] = {} if x_range is None else {'x_range': x_range}
